...
export default function CursosCrear() {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    // Validar inputs mínimo
    if (!nombreCurso || !codigoCurso || !duracionCurso || !costoCurso || !descripcion) {
    ...
    const nuevoCurso = {
      ...
      costo_curso: parseFloat(costoCurso.replace(/\D/g, '')) || 0, // Convertir costo a número
      descripcion,
    };
    ...
    fetch('http://localhost:3001/cursos/crear-curso', {
      ...
      .then(data => {
        if (data.success) {
          // Añadir el nuevo curso al estado para mostrarlo inmediatamente
          setCursos(prev => [
            ...prev,
            {
              id_curso: data.id, // Id que devuelve el backend
              ...nuevoCurso,
            },
          ]);
          // Limpiar formulario
